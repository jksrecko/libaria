...
/** @example rvisionCameraExample.cpp This is an example using the RVision
 * pan/tilt/zoom camera used on Seekur and Seekur Jr.
 *
...
//  ArFunctorC<KeyPTU> myHCB;
  ...
  bool myPTUInitRequested; ///< whether or not we've requested that the Camera initialize itself
  ArRobot *myRobot;

  ArFunctorC<KeyPTU> myDriveCB;

  bool myPowerOn; ///< whether or not we've requested the Seekur or Seekur Jr.  switch on camera power (port 12)
};
...
{
  // set the robot pointer and add a robot user task callback (called every
  // loop)
...
/* not implemented yet in ArRVision
void KeyPTU::h(void)
{
  myPTU.haltPanTilt();
  myPTU.haltZoom();
...
void KeyPTU::p(void)
{
  // toggle power state and print new status
  power(!myPowerOn);
  status();
}
